...
**Демо:** https://dogfinder.nkolesnikov.dev
...
У меня самого были две собаки. Когда я вижу на фонарном столбе объявление о
пропаже, я невольно представляю себя на месте хозяина и меня берет ужас.

## Проблема и как мой проект ее решает
Самые надёжные данные о потерявшихся собаках — исследование ASPCA 2012 года
([Weiss, Slater & Lord, *Animals*](https://doi.org/10.3390/ani2020301)). В
общенациональном опросе 1015 американских семей 14% собак хотя бы раз терялись за
предыдущие пять лет. Большинство в итоге нашлись — 93%. Показательно, каким
образом: около половины владельцев нашли собаку, обыскивая окрестности, только
15% собак вернулись благодаря адресной бирке или микрочипу, и лишь 6% владельцев
нашли питомца в приюте.

Потерявшихся собак находят рядом, соседи. Объявление на столбе — ровно этот канал,
только на бумаге: фотография, телефон и надежда, что мимо пройдёт нужный человек.
DogFinder делает то же самое, но фотография становится доступной для поиска:
нашедшему не нужно наткнуться на нужное объявление, система сама сопоставит два
объявления. Цель — оставшиеся 7%: собаки, которых хозяева так и не находят.
...
| 8 | Демо-данные, тесты, CI, production-образ, деплой | выполнено |
...
Объявления на демо-стенде расположены вокруг Вельска.
...
Повседневная работа с сервером — обновление кода, логи, бэкапы, диагностика — описана в [SERVER.md](SERVER.md).
...
Приложение и браузер обращаются к MinIO по разным адресам. Приложение ходит внутри Docker-сети по `http://minio:9000`, браузер — по публичному `https://s3.nkolesnikov.dev`.
...
Образы MinIO берутся из `quay.io`. В сентябре 2026 MinIO удалил `minio/minio` и `minio/mc` с Docker Hub после архивации community-редакции. Docker Hub отвечает на такие запросы сообщением «pull access denied», которое выглядит как проблема с авторизацией, хотя репозитория просто больше не существует.
...
SERVER.md             инструкция по работе с сервером
```
...
**Изображения в письмах на localhost.** Почтовые клиенты загружают картинки через собственные прокси-серверы, которые не имеют доступа к `localhost`. При локальной разработке в письмах вместо фотографии отображается заглушка. На публичном стенде картинки отображаются нормально.
